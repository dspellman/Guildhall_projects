...
{
	if (soundPlaybackID == 0 || soundPlaybackID == MISSING_SOUND_ID)
	{
		return false;
	}
	...
	FMOD_RESULT result = channelAssignedToSound->getCurrentSound( &soundPtr );

	if (result != FMOD_OK || !soundPtr)
	{
		return false;
	}
