...
VertexToPixelData VertexMain( VertexInputData vertex )
{
	...
	float3 cameraWorldPos = -b_viewMatrix._m30_m31_m32;
	float4 vertexLocalPos = float4( vertex.a_position, 1.0 );
	float4 vertexWorldPos = mul( b_modelMatrix, vertexLocalPos );
	float4 vertexCameraPos = mul( b_viewMatrix, vertexWorldPos );
	float4 vertexClipPos = mul( b_projectionMatrix, vertexCameraPos );
	...
	outputToPS.v_clipPos = vertexClipPos;		// <-- This is the only one D3D11 strictly requires;
	outputToPS.v_worldPos = vertexWorldPos;		// ...but we can pass on whatever else we want as well.
